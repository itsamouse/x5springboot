...
import net.jingbo.x5springboot.baas.data.sql.SQLStruct;
...
@Component
public class CRUD {
	protected static Logger logger = Logger.getLogger(CRUD.class);
	...
	//通用的sql查询
	public static JSONObject sqlQuery(JSONObject params) throws SQLException, NamingException {
		// 获取参数
	...
	public static JSONObject save(JSONObject params ) throws SQLException, NamingException, ParseException {
		// 获取参数
		String db = params.getString("db");
...
//			conn.setAutoCommit(false);
			...
			e.printStackTrace();
			logger.error(String.format("执行sql出错：%s",e.getMessage()));
			conn.rollback();
			return null;
		} finally {
			conn.close();
